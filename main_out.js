...
//                    break;
    ...
    if ("undefined" == typeof console || "undefined" == typeof DataView || "undefined" == typeof WebSocket || null == h || null == h.getContext || null == a.localStorage) alert("You browser does not support this game, we recommend you to use Firefox to play this");
		...
		$(function() {
			...
			/*
			$('body').append('<div id="chatWindow" style="position: absolute; padding: 10px; bottom: 50px; width: 280px; height: 280px"><div id="chatMessages" style="width: 100%; height: 90%; word-wrap: break-word; overflow: hidden;"></div><input type="text" id="yourMessage" placeholder="Press [ENTER] to type!" style="border: none; color: white; background-color: rgba(0, 0, 0, .2); width: 100%; padding: 8px; z-index: 1; outline: 0;"/></div>');
			_x_input = document.getElementById("yourMessage");

			_x_chatWebsocket = new WebSocket("ws://51.254.206.49:5050");
			_x_chatWebsocket.onopen = function(a) {
				$("#chatMessages").append('<div class="chatMessage"><b style="color: green">Welcome to the chat!</b></div>');
			};
			_x_chatWebsocket.onerror = function(a) {
				$("#chatMessages").append('<div class="chatMessage"><b style="color: red">Got an error.</b></div>');
				while($("#chatMessages").get(0).scrollHeight > $("#chatMessages").height()) {
					$(".chatMessage:first").remove();
				}
			};
			_x_chatWebsocket.onclose = function(a) {
				$("#chatMessages").append('<div class="chatMessage"><b style="color: orange">Chat closed.</b></div>');
				while($("#chatMessages").get(0).scrollHeight > $("#chatMessages").height()) {
					$(".chatMessage:first").remove();
				}
			};
			_x_chatWebsocket.onmessage = function(a) {
				$("#chatMessages").append(a.data);
				while($("#chatMessages").get(0).scrollHeight > $("#chatMessages").height()) {
					$(".chatMessage:first").remove();
				}
			};
